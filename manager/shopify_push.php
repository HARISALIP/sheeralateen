...
try {
    ...
    // Get the order's Shopify ID and verify branch ownership
    $stmt = $db->prepare("SELECT shopify_order_id FROM orders WHERE id = :id AND assigned_branch_id = :bid");
    $stmt->execute([':id' => $orderId, ':bid' => $branchId]);
    $shopifyOrderId = $stmt->fetchColumn();
    
    if (!$shopifyOrderId) {
        throw new Exception("This order is not linked to Shopify or not assigned to your branch.");
    }
