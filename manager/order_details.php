...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_status') {
    ...
    if ($updateOrderId > 0 && in_array($newStatus, $validStatuses) && in_array($newPaymentStatus, $validPaymentStatuses)) {
        ...
        $stmt->execute([':id' => $updateOrderId, ':bid' => $branchId]);
...
// Fetch order details, ensuring it belongs to this branch
$stmt = $db->prepare("SELECT * FROM orders WHERE id = :id AND assigned_branch_id = :bid AND deleted_at IS NULL");
